...
printfn "Day 2: Password Philosophy"
printfn ""
...
with
    static member regex = Regex(@"(?<min>[0-9]+)-(?<max>[0-9]+) (?<char>[a-z]): (?<pass>[a-z]+)", RegexOptions.Compiled)
    static member parse (s : string) : Password =
        let m = Password.regex.Match(s.Trim())
        if not m.Success then raise <| ArgumentException("Invalid password format", nameof(s))
        {
            Password.min = m.Groups.["min"].Value |> int
            max = m.Groups.["max"].Value |> int
            reqC = m.Groups.["char"].Value |> char
            pass = m.Groups.["pass"].Value
        }

let part1 (X : string) =
    let isValid (p : Password) : bool =
        let n = p.pass |> Seq.where (fun c -> c = p.reqC) |> Seq.length
        p.min <= n && n <= p.max

    let n =
        X.Split('\n')
        |> Seq.where (fun s -> not (String.IsNullOrWhiteSpace(s)))
        |> Seq.map Password.parse
        |> Seq.where isValid
        |> Seq.length
    printfn "Valid count: %d" n

let part2 (X : string) =
    let isValid (p : Password) : bool =
        p.pass.[p.min - 1] = p.reqC && p.pass.[p.max - 1] <> p.reqC
            || p.pass.[p.min - 1] <> p.reqC && p.pass.[p.max - 1] = p.reqC
